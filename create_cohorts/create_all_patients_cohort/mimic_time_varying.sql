...
FROM `roses-0.roses.mimic_eligibility_24h_all_patients` AS el -- for 24h dataset
LEFT JOIN `physionet-data.mimiciv_icu.icustays` AS icustays ON el.stay_id = icustays.stay_id
LEFT JOIN `roses-0.roses.baseline_mimic_24h_all_patients` as baseline_24h ON baseline_24h.stay_id = el.stay_id
WHERE eligible = 1),
...
SELECT *
FROM `roses-0.roses.mimic_inputevents_all_patients` -- 24h dataset, changed nov 8
...
left join `roses-0.roses.mimic_eligibility_24h_all_patients` AS el on el.stay_id = first.stay_id 
-- above line was changed nov 8, confirm correct
