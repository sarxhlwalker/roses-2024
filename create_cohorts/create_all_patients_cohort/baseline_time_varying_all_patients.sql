...
from `roses-0.roses.baseline_mimic_24h_all_patients` as baseline
left join (
  select stay_id, time_interval, value as heart_rate
  from `roses-0.roses.mimic_time_varying_all_patients`
...
from second_pass
left join `roses-0.roses.mimic_time_varying_all_patients` as prop on prop.stay_id = second_pass.stay_id and prop.time_interval < 0 and prop.variable in ("Propofol")
left join (
        ...
        from `roses-0.roses.mimic_time_varying_all_patients` as time
