...
, next as (select *, 
case
    when race_clean is null or race_clean = "INDIGENOUS" then "OTHER"
    else race_clean
end as race_clean2, 
case 
	when ventilation_inclusion = 1 and gender_inclusion = 1 and (death_offset_days >= 0 or death_offset_days is null) and trach = 0 and time_zero is not null and is_first = 1 then 1 
    else 0 
end as eligible
from eleven
ORDER BY eligible DESC)

select stay_id, hadm_id, subject_id, female_clean, gender_inclusion, race_inclusion, race_clean2 as race_clean, time_zero, trach, death_offset_days, ventilation_inclusion, data_source, eligible
from next 
