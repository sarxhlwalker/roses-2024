...
saveRDS(test, 'quantiles/outcome_quantile_all_patients.RDS')
q <- readRDS('quantiles/outcome_quantile_all_patients.RDS')
...
saveRDS(mean_sd, file = 'quantiles/mean_sd_all_patients.RDS')
...
complete_data <- lapply(complete_data, function (df) {
  q2 <- readRDS('quantiles/outcome_quantile_all_patients.RDS')
  df2 <- df %>% mutate(
    propofol_outcome_quantile = case_when(
      propofol_outcome_divided <= 0 ~ 1, 
...
factor_categorical_vars <- function(complete_tables){
  for(i in 1:length(complete_tables)){
    complete_tables[[i]] <- complete_tables[[i]] %>%
      mutate(
        other = ifelse(white == 0 & black == 0 & asian == 0 & hispanic == 0, 
                       1, 0)
      ) %>% 
...
#   fill(everything(), .direction = "down") %>%
#   ungroup()
# saveRDS(fill, paste0('../results/r/table_all_patients.RDS'))
